...
<div class="p-4 bg-gray-50">
    <nav class="text-sm text-gray-500">
        <ol class="list-reset flex">
            ...
            <li>
                <a href="{{ route('package.index') }}" class="text-blue-600 hover:text-blue-700 font-medium">Package</a>
            </li>
            <li><span class="mx-2">/</span></li>
            <li class="text-gray-700">Edit Package</li>
        </ol>
    </nav>
</div>
...
<div>
    <div class="w-full lg:w-1/2 flex flex-col max-h-[90%] mx-auto bg-white rounded-3xl shadow-lg overflow-hidden">
        ...
        <div class="px-8 py-6 overflow-y-auto h-[90%]">
            <form id="dataForm" action="{{ route('package.update') }}" method="POST" enctype="multipart/form-data">
                @method('put')
                @csrf
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="text" placeholder="Enter title..."
                                value="{{ $data['package']->title ?? '' }}" name="title" required
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="text" placeholder="Enter author name..."
                                value="{{ $data['package']->author ?? '' }}" name="author"
                                class="w-full px-4 py-2.5 text-sm text-gray-700 bg-white border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm hover:shadow-md placeholder-gray-400">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select name="status"
                                ...
                                <option value="1" {{ $data['package']->status == '1' ? 'selected' : '' }}>Active
                                </option>
                                <option value="0" {{ $data['package']->status == '0' ? 'selected' : '' }}>InActive
                                </option>
                                <option value="-1" {{ $data['package']->status == '-1' ? 'selected' : '' }}>
                                    Not Approved</option>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" placeholder="Enter Sort..." name="sort" value="{{ $data['package']->sort ?? 0 }}" required
                                class="w-full px-4 py-2.5 text-sm text-gray-700 bg-white border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm hover:shadow-md placeholder-gray-400">
                        </div>
                    </div>
                    ...
                    <div class="col-span-2">
                        ...
                        <div class="relative">
                            <input type="file" name="image" id="image" data-previewSectionId="image-preview"
                                accept="image/*" class="hidden image-upload&preview" />
                            <label for="image"
                    ...
                    <!--=== Image1 Preview Section ====-->
                    <div id="image-preview" class="{{ isset($data['package']->image) ? '' : 'hidden' }}  col-span-2">
                        @if (isset($data['package']->image))
                        <div class="relative inline-flex gap-2">
                            <img src="{{ asset('storage/' . $data['package']->image) }}"
                    ...
                    <div class="col-span-2">
                        <label class=" text-xs font-semibold text-gray-700 mb-2 flex items-center gap-1.5">
                            <img src="{{ asset('assets/svg/file-text.svg') }}" class="w-3.5 h-3.5 pointer-events-none"
                                alt="">
                            Content
                        </label>
                        <div class="relative">
                            <textarea type="text" name="content" required placeholder="Enter content..."
                                class="w-full px-4 py-2.5 text-sm text-gray-700 bg-white border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm hover:shadow-md placeholder-gray-400">{{ $data['package']->content ?? '' }}</textarea>
                        </div>
                    </div>
                </div>
