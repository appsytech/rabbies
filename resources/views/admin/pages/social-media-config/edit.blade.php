...
@section('title', 'Social Media Config')
    ...
    <div class="p-4 bg-gray-50">
        <nav class="text-sm text-gray-500">
            <ol class="list-reset flex">
                ...
                <li>
                    <a href="{{ route('social-media-config.index') }}"
                        class="text-blue-600 hover:text-blue-700 font-medium">Social Media Config</a>
                </li>
                <li><span class="mx-2">/</span></li>
                <li class="text-gray-700">Edit Social Media Config</li>
            </ol>
        </nav>
    </div>
    ...
    <div>
        <div class="w-full lg:w-1/2 flex flex-col max-h-[90%] mx-auto bg-white rounded-3xl shadow-lg overflow-hidden">
            ...
            <div class="px-8 py-6 overflow-y-auto h-[90%]">
                <form id="dataForm" action="{{ route('social-media-config.update') }}" method="POST"
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
                        ...
                        <div>
                            <label class="text-xs font-semibold text-gray-700 mb-2 flex items-center gap-1.5">
                                <img src="{{ asset('assets/svg/share-two.svg') }}" class="w-3.5 h-3.5 pointer-events-none"
                                    alt="">
                                Name
                            </label>
                            <div class="relative">
                                <input type="text" value="{{ $data['config']->name ?? '' }}" placeholder="Enter Name..."
                                    name="name" required
                       ...
                       {{-- ====== Type ====== --}}
                            <div>
                                ...
                                <div class="relative">
                                    <select name="type" required
                                        ...
                                        <option value="facebook" {{ $data['config']->type == 'facebook' ? 'selected' : '' }}>Facebook</option>
                                        <option value="instagram"  {{ $data['config']->type == 'instagram' ? 'selected' : '' }}>Instagram</option>
                                        <option value="twitter" {{ $data['config']->type == 'twitter' ? 'selected' : '' }}>Twitter</option>
                                        <option value="linkedin" {{ $data['config']->type == 'linkedin' ? 'selected' : '' }}>Linkedin</option>
                                        <option value="youtube" {{ $data['config']->type == 'youtube' ? 'selected' : '' }}>Youtube</option>
                                    </select>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="text" value="{{ $data['config']->link ?? '' }}"
                                    placeholder="Enter Social Media Url..." name="link" required
                                    class="w-full px-4 py-2.5 text-sm text-gray-700 bg-white border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm hover:shadow-md placeholder-gray-400">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="number" value="{{ $data['config']->sort ?? '' }}"
                                    placeholder="Enter Sort Order.." name="sort" required
                                    class="w-full px-4 py-2.5 text-sm text-gray-700 bg-white border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm hover:shadow-md placeholder-gray-400">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <select name="status" required
                                    class="w-full px-4 py-2.5 text-sm text-gray-700 bg-white border-2 border-gray-200
                                    rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500
                                    transition-all shadow-sm hover:shadow-md appearance-none cursor-pointer">

                                    <option value="1" {{ $data['config']->status == '1' ? 'selected' : '' }}>Active
                                    </option>
                                    <option value="0" {{ $data['config']->status == '0' ? 'selected' : '' }}>Inactive
                                    </option>
                                </select>
                                <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                                    <img src="{{ asset('assets/svg/cheveron-down.svg') }}"
